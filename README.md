...
| [.easignore](./.easignore)                                         | Same syntax as `.gitignore`; trims the EAS build tarball (store assets, docs, `web/`, tests, `*.p8`) so uploads stay small + secret-free. |
| [eas.json.template](./eas.json.template)                           | Reference `eas.json` with `submit` blocks for iOS (`ascAppId`) and Android (service-account key). Copy into `eas.json` after `eas init`.  |
...
| [fastlane/README.md](./fastlane/README.md)                                                 | Automate iOS + Android store listings (text + screenshots) via ASC API / Play supply; verify with the `status` scripts.      |
| [fastlane/scripts/asc-status.rb](./fastlane/scripts/asc-status.rb)                         | Read-only ASC query — confirms what's actually live on App Store Connect (don't trust fastlane's noisy logs).                |
| [fastlane/scripts/play-status.rb](./fastlane/scripts/play-status.rb)                       | Read-only Play query — confirms app existence and track state via the Android Publisher API.                                 |
| [fastlane/scripts/check-play-metadata.sh](./fastlane/scripts/check-play-metadata.sh)       | Lint Play metadata lengths (title ≤30, short ≤80, full ≤4000, changelog ≤500) before uploading.                              |
